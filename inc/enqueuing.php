 ...
 */
function studio25_enqueue_global_styles(): void {
	studio25_enqueue_style_asset( 'studio25-global-style', 'build/global-styles.css', ! is_admin() );
}
add_action( 'enqueue_block_assets', 'studio25_enqueue_global_styles' );

/**
 * Enqueue the public-facing stylesheet.
 */
function studio25_enqueue_frontend_styles(): void {
	studio25_enqueue_style_asset( 'studio25-screen-style', 'build/screen.css' );
}
 ...
 */
function studio25_register_editor_styles(): void {
	add_editor_style( 'build/editor.css' );
}
add_action( 'after_setup_theme', 'studio25_register_editor_styles' );
 ...
 */
function studio25_register_block_assets(): void {
	$blocks_dir = get_theme_file_path( 'build/blocks' );
	if ( ! is_dir( $blocks_dir ) ) {
		return;
	}

	// Scan for namespace directories (core, woocommerce, studio25, etc.)
	foreach ( glob( $blocks_dir . '/*', GLOB_ONLYDIR ) as $namespace_path ) {
		...
		foreach ( glob( $namespace_path . '/*', GLOB_ONLYDIR ) as $block_path ) {
			...
			if ( file_exists( $base_style_file ) ) {
				$handle   = studio25_build_block_style_handle( $block_name, 'base' );
				$relative = "build/blocks/{$namespace}/{$block_slug}/style.css";
				$asset    = studio25_read_asset_file( $base_style_file );
			...
			if ( is_dir( $styles_dir ) ) {
				foreach ( glob( $styles_dir . '/*.css' ) as $css_file ) {
					...
					$handle     = studio25_build_block_style_handle( $block_name, $style_slug );
					$relative   = "build/blocks/{$namespace}/{$block_slug}/styles/{$style_slug}.css";
					$asset      = studio25_read_asset_file( $css_file );
...
add_action( 'init', 'studio25_register_block_assets', 9 );
 ...
 */
function studio25_enqueue_block_editor_scripts(): void {
	$blocks_dir = get_theme_file_path( 'build/blocks' );
	if ( ! is_dir( $blocks_dir ) ) {
		return;
	}
	...
	foreach ( glob( $blocks_dir . '/*', GLOB_ONLYDIR ) as $namespace_path ) {
		...
		foreach ( glob( $namespace_path . '/*', GLOB_ONLYDIR ) as $block_path ) {
			...
			if ( file_exists( $editor_js ) ) {
				$handle   = "studio25-block-{$namespace}-{$block_slug}-editor";
				$relative = "build/blocks/{$namespace}/{$block_slug}/editor.js";

				studio25_enqueue_script_asset( $handle, $relative, false );
			}
		}
	}
}
add_action( 'enqueue_block_editor_assets', 'studio25_enqueue_block_editor_scripts' );
 ...
 */
function studio25_autoload_block_php_files(): void {
	$blocks_dir = get_theme_file_path( 'build/blocks' );
	if ( ! is_dir( $blocks_dir ) ) {
		return;
	}
...
add_action( 'after_setup_theme', 'studio25_autoload_block_php_files', 5 );
 ...
 */
function studio25_autoload_includes_php_files(): void {
...
add_action( 'after_setup_theme', 'studio25_autoload_includes_php_files', 5 );
 ...
 */
function studio25_autoload_plugin_php_files(): void {
	...
	foreach ( glob( $plugins_dir . '/*', GLOB_ONLYDIR ) as $plugin_path ) {
		...
		if ( ! studio25_is_plugin_active( $plugin_slug ) ) {
			continue;
		}
...
add_action( 'after_setup_theme', 'studio25_autoload_plugin_php_files', 99 );
 ...
 */
function studio25_get_includes_component_map(): array {
	static $cache = null;
	if ( null !== $cache ) {
		return $cache;
	}
 ...
 * @return bool
 */
function studio25_is_plugin_active( string $slug ): bool {
 ...
 */
function studio25_get_plugins_component_map(): array {
	static $cache = null;
	if ( null !== $cache ) {
		return $cache;
	}
 ...
 */
function studio25_enqueue_plugin_assets(): void {
	$components = studio25_get_plugins_component_map();

	foreach ( $components as $slug => $component ) {
		// Check if plugin is active
		if ( studio25_is_plugin_active( $slug ) ) {
			studio25_enqueue_plugin_component( $slug );
		}
	}
}
add_action( 'wp_enqueue_scripts', 'studio25_enqueue_plugin_assets' );
 ...
 * @param string $slug Plugin slug.
 */
function studio25_enqueue_plugin_component( string $slug ): void {
 ...
 */
function studio25_enqueue_includes_in_editor(): void {
	if ( ! is_admin() ) {
		return;
	}

	$components = studio25_get_includes_component_map();
	foreach ( $components as $key => $component ) {
		studio25_enqueue_include_component( $key );
	}

	// Also enqueue plugin assets in editor
	$plugins = studio25_get_plugins_component_map();
...
add_action( 'enqueue_block_assets', 'studio25_enqueue_includes_in_editor' );

/**
 * Enqueue all block style variation handles in the editor.
 */
function studio25_enqueue_block_styles_in_editor(): void {
	if ( ! is_admin() ) {
		return;
	}
	...
	foreach ( glob( $blocks_dir . '/*', GLOB_ONLYDIR ) as $namespace_path ) {
		...
		foreach ( glob( $namespace_path . '/*', GLOB_ONLYDIR ) as $block_path ) {
			...
			$base_handle = studio25_build_block_style_handle( $block_name, 'base' );
...
add_action( 'enqueue_block_assets', 'studio25_enqueue_block_styles_in_editor' );
 ...
 * @return string
 */
function studio25_enqueue_dynamic_assets( string $block_content, array $block ): string {
	if ( is_admin() ) {
		return $block_content;
	}

	studio25_maybe_enqueue_block_style_variations( $block );
	studio25_maybe_enqueue_include_assets( $block );

	return $block_content;
}
add_filter( 'render_block', 'studio25_enqueue_dynamic_assets', 10, 2 );
 ...
 * @param array $block Block metadata.
 */
function studio25_maybe_enqueue_block_style_variations( array $block ): void {
	...
	foreach ( $matches[1] as $style_slug ) {
		$handle = studio25_build_block_style_handle( $block['blockName'], $style_slug );
		if ( wp_style_is( $handle, 'registered' ) ) {
			wp_enqueue_style( $handle );
		}
	}
 ...
 * Plugins are handled separately via studio25_enqueue_plugin_includes().
 *
 * @param array $block Block metadata.
 */
function studio25_maybe_enqueue_include_assets( array $block ): void {
	$components = studio25_get_includes_component_map();
	if ( empty( $components ) ) {
		return;
	}
	...
	$slugs_to_enqueue = apply_filters(
		'studio25_include_component_slugs',
		$slugs_to_enqueue,
		$block,
		$components
	);
 ...
 */
function studio25_enqueue_include_component( string $key ): void {
	static $enqueued = array();
	...
	foreach ( glob( $component_dir . '/*.css' ) as $css_file ) {
		...
		$filename = basename( $css_file, '.css' );
		$handle   = "studio25-include-{$component['category']}-{$component['name']}-{$filename}";
		$relative = "{$relative_base}/{$filename}.css";
	...
	$view_js = $component_dir . '/view.js';
	if ( file_exists( $view_js ) && ! is_admin() ) {
		$handle   = "studio25-include-{$component['category']}-{$component['name']}-view";
		$relative = "{$relative_base}/view.js";
		studio25_enqueue_script_asset( $handle, $relative );
	}
	...
	$editor_js = $component_dir . '/editor.js';
	if ( file_exists( $editor_js ) && is_admin() ) {
		$handle   = "studio25-include-{$component['category']}-{$component['name']}-editor";
		$relative = "{$relative_base}/editor.js";
		studio25_enqueue_script_asset( $handle, $relative, false );
	}
}
 ...
 */
function studio25_enqueue_style_asset( string $handle, string $relative_path, bool $async = false ): void {
	$file_path = get_theme_file_path( $relative_path );
	if ( ! file_exists( $file_path ) ) {
		return;
	}

	$asset = studio25_read_asset_file( $file_path );
 ...
 */
function studio25_enqueue_script_asset( string $handle, string $relative_path, bool $defer = true ): void {
	$file_path = get_theme_file_path( $relative_path );
	if ( ! file_exists( $file_path ) ) {
		return;
	}
 ...
 */
function studio25_read_asset_file( string $file_path ): array {
 ...
 * @return string
 */
function studio25_build_block_style_handle( string $block_name, string $style_slug ): string {
	return 'studio25-block-style-' . str_replace( '/', '-', $block_name ) . '-' . $style_slug;
}
